...
class GSLRec(nn.Module):
    def __init__(self, config, dataset: Data, uu_graph, ii_graph, mu_u, mu_i, device):
        ...
        self.user_gcns = GraphConv(config.in_size, config.out_size, norm='both',
                                 weight=False, bias=False, activation=None, allow_zero_in_degree=True)

        self.item_gcns = GraphConv(config.in_size, config.out_size, norm='both',
                                 weight=False, bias=False, activation=None, allow_zero_in_degree=True)
    ...
    def bpr_loss(self, user, positive, negative):
        ...
        for i in range(2):
            if i == 0:
                user_emb1 = self.user_gcns(self.uu_graph, self.user_embedding.weight).flatten(1)
                item_emb1 = self.item_gcns(self.ii_graph, self.item_embedding.weight).flatten(1)
            else:
                user_emb1 = self.user_gcns(self.uu_graph, user_emb1).flatten(1)
                item_emb1 = self.item_gcns(self.ii_graph, item_emb1).flatten(1)
            user_emb.append(user_emb1)
            item_emb.append(item_emb1)
        user_emb1 = torch.stack(user_emb, dim=1)
        item_emb1 = torch.stack(item_emb, dim=1)
        user_emb1 = torch.mean(user_emb1, dim=1)
        item_emb1 = torch.mean(item_emb1, dim=1)

        all_user_embeddings = 0.5*user_emb0 + 0.5*user_emb1
        all_item_embeddings = 0.5*item_emb0 + 0.5*item_emb1
